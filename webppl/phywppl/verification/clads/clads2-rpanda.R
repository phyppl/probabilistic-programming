...
for(i in 1:nrow(param)) {
  ...
  reparam = list( param = c(sigma = param[i,]$sigma, alpha = param[i,]$alpha, mu = param[i,]$epsilon * param[i, ]$lambda, lambda = rep(param[i,]$lambda, nedges + 1)), f = param[i,]$f)
...
write.table(data.frame(param, likelihood_est), file = "verification/clads/clads2-rpanda-results.csv")
