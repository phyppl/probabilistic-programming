class ClaDS2Model < PhyModel<PhyNode, ClaDS2Parameter> {
  λ_k:Real;
  λ_θ:Real;
  ε_min:Real;
  ε_max:Real;
  ...
  fiber step() -> Event {
    switchToParameter(ClaDS2Parameter(θ));  // Each branch has its own parameter
    θ.ref <- node.index;
    ...
    while !last {
      ...
      0 ~> Poisson(θ.λ * θ.λ_factor * θ.ε * (seg_beg - seg_end));
  ...
  fiber simulateUnobserved(t_beg:Real, λ_factor_parent:Real) -> Event {
    ...
    Δ ~ Exponential(θ.λ * λ_factor * (1+θ.ε));
    if Δ > t_beg {
      ...
      speciation ~ Bernoulli(1.0/(1+θ.ε));
      if speciation {
